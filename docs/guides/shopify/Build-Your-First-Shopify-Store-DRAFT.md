...
As of the July 2026 snapshot, Lucid Creations used nineteen collections and a two-level Shop menu. The main navigation was Home, a Shop menu that opened to product collections (including T-Shirts, Hoodies & Sweatshirts, Blankets, Tapestries, Hats, Pillows, Journals & Paper Goods, Accessories, Wall Art, Best Sellers, and Sacred Geometry Art), Commissions, an RJ Print page, and Contact. Most collections were manual; two were automatic — one that gathered all products by vendor and one that gathered drinkware by product type.
...
The Best Sellers collection was a manually chosen list, not one built from an actual sales rule. A name like Best Sellers implies real sales data, so it should either be driven by a genuine sales-based condition or be renamed to show that it is a curated selection. This guide makes no claim about the store’s actual sales.
...
A controlled test uses fictional information only: a test product and variant, a controlled inbox or an example address for notifications, and the official simulated-payment method so no real money moves. Before testing, confirm whether the store is already accepting real orders and whether any connected app could email, fulfill, print a shipping label, or send work to a print-on-demand provider — and prevent those real actions unless a real-transaction test is deliberately authorized.
...
After testing, turn off test mode, restore any temporary settings, and confirm the intended live state. No real order numbers, addresses, payment details, customer records, or credentials belong in a test writeup or in published screenshots — use fictional, clearly labeled data throughout.
