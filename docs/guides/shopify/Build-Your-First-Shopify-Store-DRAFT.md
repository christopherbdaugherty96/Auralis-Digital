...
6. **Activate the selected provider.**
   ...
   - **Enter:** in the secure official provider interface only, the required identity or business verification information, then submit the setup.
   ...
   - **Done when:** the provider is active — or you have recorded any pending verification, hold, or the exact remaining requirement — and you have confirmed the provider's status in Shopify. This is what actually lets you take money.
   `[VERIFY CURRENT SHOPIFY SETTING]`
...
15. **Enable test mode.** Confirm the provider setup is complete enough for testing; open the payment-provider settings; enable the provider's official test or sandbox mode; confirm the admin displays a visible test-mode indicator; confirm no live customers are using the store; and record the time testing began. *Why it matters:* test mode lets you simulate payments without real charges. *Done when:* a visible test-mode indicator is active. You can repeat this test as many times as you need using Shopify’s official test method; test orders are simulated and do not charge real money.
    `[VERIFY CURRENT SHOPIFY SETTING]`
...
This chapter is testing: you walk through your store the way a customer would, using Shopify’s official test method so no real money moves. The tests are safe to repeat as many times as you need.
